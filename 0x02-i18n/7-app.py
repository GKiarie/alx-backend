...
def get_timezone():
    """get timezone"""
    # First, check if the timezone is provided in URL parameters
    user_timezone = request.args.get("timezone")
    if user_timezone and is_valid_timezone(user_timezone):
        return user_timezone
    # Next, check if the user has a preferred timezone in their settings
    user = getattr(g, 'user', None)
    if user and user.get('timezone') and is_valid_timezone(user['timezone']):
        return user['timezone']
    # If no preferred timezone is found, default to UTC
    return 'UTC'
